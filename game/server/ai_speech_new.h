...
{
public:
	virtual void OnSpokeConcept( AIConcept_t conceptId, AI_Response *response )	{};
...
public:
	...
	bool Speak( AIConcept_t conceptId, const char *modifiers = NULL, char *pszOutResponseChosen = NULL, size_t bufsize = 0, IRecipientFilter *filter = NULL );
	bool Speak( const AIConcept_t &conceptId, AI_CriteriaSet *criteria, char *pszOutResponseChosen = NULL, size_t bufsize = 0, IRecipientFilter *filter = NULL );
	...
	void GatherCriteria( AI_CriteriaSet *outputCritera, const AIConcept_t &conceptId, const char *modifiers );
	// These two methods allow looking up a response and dispatching it to be two different steps
	// AI_Response *SpeakFindResponse( AIConcept_t conceptId, const char *modifiers = NULL );
	// AI_Response *SpeakFindResponse( AIConcept_t &conceptId, AI_CriteriaSet *criteria );
	...
	bool FindResponse( AI_Response &outResponse, const AIConcept_t &conceptId, AI_CriteriaSet *modifiers = NULL );
	virtual bool SpeakDispatchResponse( AIConcept_t conceptId, AI_Response *response, AI_CriteriaSet *criteria, IRecipientFilter *filter = NULL );
	...
	bool CanSpeakConcept( const AIConcept_t &conceptId );
	bool SpokeConcept( const AIConcept_t &conceptId );
	float GetTimeSpokeConcept( const AIConcept_t &conceptId ); // returns -1 if never
	void SetSpokeConcept( const AIConcept_t &conceptId, AI_Response *response, bool bCallback = true );
	void ClearSpokeConcept( const AIConcept_t &conceptId );
	...
	bool ScriptSpeak( char const *conceptId, const char *modifiers ) { return Speak( conceptId, modifiers[0] != '\0' ? modifiers : NULL ); }
#endif
...
public:
	...
	virtual bool 	Speak( AIConcept_t conceptId, const char *modifiers = NULL, char *pszOutResponseChosen = NULL, size_t bufsize = 0, IRecipientFilter *filter = NULL );
	virtual bool 	Speak( AIConcept_t conceptId, AI_CriteriaSet *pCriteria, char *pszOutResponseChosen = NULL, size_t bufsize = 0, IRecipientFilter *filter = NULL );
#ifdef MAPBASE
	virtual bool	Speak( AIConcept_t conceptId, AI_CriteriaSet& modifiers, char *pszOutResponseChosen = NULL, size_t bufsize = 0, IRecipientFilter *filter = NULL ) { return Speak( conceptId, &modifiers, pszOutResponseChosen, bufsize, filter ); }
#endif
	...
	// These two methods allow looking up a response and dispatching it to be two different steps
#ifdef MAPBASE
	//AI_Response *SpeakFindResponse( AIConcept_t conceptId, const AI_CriteriaSet& modifiers );
	inline bool	SpeakDispatchResponse( const AIConcept_t &conceptId, AI_Response &response, AI_CriteriaSet *criteria = NULL ) { return SpeakDispatchResponse( conceptId, &response, criteria ); }
#endif
	bool SpeakFindResponse( AI_Response& outResponse, const AIConcept_t &conceptId, const char *modifiers = NULL );
	// AI_Response *	SpeakFindResponse( AIConcept_t conceptId, const char *modifiers = NULL );
	// AI_Response *SpeakFindResponse( AIConcept_t conceptId, AI_CriteriaSet *criteria );
	// AI_Response *SpeakFindResponse( AIConcept_t conceptId );
	// Find the appropriate response for the given concept. Return false if none found.
	// Fills out the response object that you provide.
	bool FindResponse( AI_Response &outResponse, const AIConcept_t &conceptId, AI_CriteriaSet *criteria = NULL );

	bool 			SpeakDispatchResponse( AIConcept_t conceptId, AI_Response *response,  AI_CriteriaSet *criteria = NULL );
	virtual void	PostSpeakDispatchResponse( AIConcept_t conceptId, AI_Response *response ) { return; }
	float 			GetResponseDuration( AI_Response *response );
	...
	void SetSpokeConcept( AIConcept_t conceptId, AI_Response *response, bool bCallback = true ) 		{ this->GetExpresser()->SetSpokeConcept( conceptId, response, bCallback ); }
	float GetTimeSpokeConcept( AIConcept_t conceptId )												{ return this->GetExpresser()->GetTimeSpokeConcept( conceptId ); }
	bool SpokeConcept( AIConcept_t conceptId )														{ return this->GetExpresser()->SpokeConcept( conceptId ); }
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::Speak( AIConcept_t conceptId, const char *modifiers /*= NULL*/, char *pszOutResponseChosen /*=NULL*/, size_t bufsize /* = 0 */, IRecipientFilter *filter /* = NULL */ )
{
	AssertOnce( this->GetExpresser()->GetOuter() == this );
	return this->GetExpresser()->Speak( conceptId, modifiers, pszOutResponseChosen, bufsize, filter );
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::Speak( AIConcept_t conceptId, AI_CriteriaSet *pCriteria, char *pszOutResponseChosen /*=NULL*/, size_t bufsize /* = 0 */, IRecipientFilter *filter /* = NULL */ )
{
	AssertOnce( this->GetExpresser()->GetOuter() == this );
	CAI_Expresser * const RESTRICT pExpresser = this->GetExpresser();
	conceptId.SetSpeaker(this);
	// add in any local criteria to the one passed on the command line.
	pExpresser->GatherCriteria( pCriteria, conceptId, NULL );
	// call the "I have aleady gathered criteria" version of Expresser::Speak
	return pExpresser->Speak( conceptId, pCriteria, pszOutResponseChosen, bufsize, filter );
}
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline void CAI_ExpresserHost<BASE_NPC>::GatherCriteria( AI_CriteriaSet *outputCriteria, const AIConcept_t &conceptId, const char *modifiers )
{
	return this->GetExpresser()->GatherCriteria( outputCriteria, conceptId, modifiers );
}
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::SpeakFindResponse(AI_Response& outResponse, const AIConcept_t &conceptId, const char *modifiers /*= NULL*/ )
{
	AI_CriteriaSet criteria;
	GatherCriteria(&criteria, conceptId, modifiers);
	return FindResponse( outResponse, conceptId, &criteria );
}
#else
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline AI_Response *CAI_ExpresserHost<BASE_NPC>::SpeakFindResponse( const AIConcept_t &conceptId, const char *modifiers /*= NULL*/ )
{
	return this->GetExpresser()->SpeakFindResponse( conceptId, modifiers );
}
#endif
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline AI_Response *CAI_ExpresserHost<BASE_NPC>::SpeakFindResponse( const AIConcept_t &conceptId, AI_CriteriaSet *criteria /*= NULL*/ )
{
	return this->GetExpresser()->SpeakFindResponse( conceptId, criteria );
}
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline AI_Response * CAI_ExpresserHost<BASE_NPC>::SpeakFindResponse( const AIConcept_t &conceptId )
{
	AI_CriteriaSet criteria;
	GatherCriteria( &criteria, conceptId, NULL );
	return this->GetExpresser()->SpeakFindResponse( conceptId, &criteria );
}
#endif
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::FindResponse( AI_Response &outResponse, const AIConcept_t &conceptId, AI_CriteriaSet *criteria )
{
	return this->GetExpresser()->FindResponse( outResponse, conceptId, criteria );
}
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::SpeakDispatchResponse( AIConcept_t conceptId, AI_Response *response,  AI_CriteriaSet *criteria )
{
	if ( this->GetExpresser()->SpeakDispatchResponse( conceptId, response, criteria ) )
	{
		PostSpeakDispatchResponse( conceptId, response );
		return true;
	}
...
protected:
	static void DispatchFollowupThroughQueue( const AIConcept_t &conceptId,
