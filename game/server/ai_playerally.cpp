...
public:
	CConceptInfoMap() :
...
#endif
	  {
		  ...
		  {
			  Insert( g_ConceptInfos[i].conceptId, &g_ConceptInfos[i] );
		  }
	  }
};
...
void CAI_AllySpeechManager::Spawn()
{
	...
	for ( int i = 0; i < ARRAYSIZE(g_ConceptInfos); i++ )
		m_ConceptTimers.Insert( AllocPooledString( g_ConceptInfos[i].conceptId ), CSimpleSimTimer() );
}
...
void CAI_AllySpeechManager::AddCustomConcept( const ConceptInfo_t &conceptInfo )
{
	...
	if ( g_ConceptInfoMap.Find( conceptInfo.conceptId ) == g_ConceptInfoMap.InvalidIndex() )
	{
		g_ConceptInfoMap.Insert( conceptInfo.conceptId, new ConceptInfo_t( conceptInfo ) );
	}

	if ( m_ConceptTimers.Find( AllocPooledString(conceptInfo.conceptId) ) == m_ConceptTimers.InvalidIndex() )
	{
		m_ConceptTimers.Insert( AllocPooledString( conceptInfo.conceptId ), CSimpleSimTimer() );
	}
}
...
ConceptInfo_t *CAI_AllySpeechManager::GetConceptInfo( AIConcept_t conceptId )
{
	int iResult = g_ConceptInfoMap.Find( conceptId );

	return ( iResult != g_ConceptInfoMap.InvalidIndex() ) ? g_ConceptInfoMap[iResult] : NULL;
}

void CAI_AllySpeechManager::OnSpokeConcept( CAI_PlayerAlly *pPlayerAlly, AIConcept_t conceptId, AI_Response *response )
{
	ConceptInfo_t *			pConceptInfo	= GetConceptInfo( conceptId );
		...
		if ( pConceptInfo->flags & AICF_PROPAGATE_SPOKEN )
		{
			...
			for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
			{
					 ...
					 pPlayerAlly->FVisible( pTalker ) )
				{
					...
					pTalker->GetExpresser()->SetSpokeConcept( conceptId, NULL, false );
				}
			}
		}
	}
	...
	if ( !ai_no_talk_delay.GetBool() )
	{
		...
		if ( pConceptInfo && pConceptInfo->minConceptDelay != -1 )
		{
			...
			char iConceptTimer = m_ConceptTimers.Find( MAKE_STRING(conceptId) );
...
bool CAI_AllySpeechManager::ConceptDelayExpired( AIConcept_t conceptId )
{
	char iConceptTimer = m_ConceptTimers.Find( MAKE_STRING(conceptId) );
	if ( iConceptTimer != m_ConceptTimers.InvalidIndex() )
...
#ifdef NEW_RESPONSE_SYSTEM
				SpeakDispatchResponse( selection.conceptId.c_str(), &selection.Response );
#else
				SpeakDispatchResponse( selection.conceptId.c_str(), selection.pResponse );
#endif
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::SelectSpeechResponse( AIConcept_t conceptId, const char *pszModifiers, CBaseEntity *pTarget, AISpeechSelection_t *pSelection )
{
	if ( IsAllowedToSpeak( conceptId ) )
	{
#ifdef NEW_RESPONSE_SYSTEM
		bool result = SpeakFindResponse( pSelection->Response, conceptId, pszModifiers );
		if ( result )
		{
			pSelection->conceptId = conceptId;
			pSelection->hSpeechTarget = pTarget;
			return true;
		}
#else
		AI_Response *pResponse = SpeakFindResponse( conceptId, pszModifiers );
		if ( pResponse )
		{
			pSelection->Set( conceptId, pResponse, pTarget );
			return true;
		}
#endif
	}
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::PostSpeakDispatchResponse( AIConcept_t conceptId, AI_Response *response )
{
#ifdef HL2_EPISODIC
	CAI_AllySpeechManager *pSpeechManager = GetAllySpeechManager();
	ConceptInfo_t *pConceptInfo	= pSpeechManager->GetConceptInfo( conceptId );
	if ( pConceptInfo && (pConceptInfo->flags & AICF_QUESTION) && GetSpeechTarget() )
	{
		bool bSaidHelloToNPC = !Q_strcmp(conceptId, "TLK_HELLO_NPC");
		...
		if ( rr_debug_qa.GetBool() )
		{
			if ( bSaidHelloToNPC )
			{
				Warning("Q&A: '%s' said Hello to '%s' (concept %s)\n", GetDebugName(), GetSpeechTarget()->GetDebugName(), (const char*)conceptId );
			}
			else
			{
...
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::AskQuestionNow( CBaseEntity *pSpeechTarget, int iQARandomNumber, const char *conceptId )
{
	...
	AISpeechSelection_t selection;
#ifdef NEW_RESPONSE_SYSTEM
	if (SelectSpeechResponse( conceptId, NULL, m_hPotentialSpeechTarget.Get(), &selection ))
	{
		SetSpeechTarget( selection.hSpeechTarget );
		ClearPendingSpeech();

		// Speak immediately
		return SpeakDispatchResponse( selection.conceptId.c_str(), &selection.Response );
	}

	return false;
#else
	SelectSpeechResponse( conceptId, NULL, m_hPotentialSpeechTarget.Get(), &selection );
	...
	// Speak immediately
	return SpeakDispatchResponse( selection.conceptId.c_str(), selection.pResponse );
#endif
}
...
void CAI_PlayerAlly::InputAskQuestion( inputdata_t &inputdata )
{
	...
	const char *conceptId = TLK_QUESTION;
	...
	FOR_EACH_VEC( vecStrings, i )
	{
		...
		switch (i)
		{
			...
			case 2:		conceptId = vecStrings[i]; break;
		}
	}
	...
	AskQuestionNow(pSpeechTarget, iQARandomNumber, conceptId);
}
#endif
...
int CAI_PlayerAlly::SelectNonCombatSpeechSchedule()
{
	if ( !HasPendingSpeech() )
	{
		...
		if ( SelectNonCombatSpeech( &selection ) )
		{
			SetSpeechTarget( selection.hSpeechTarget );
#ifdef NEW_RESPONSE_SYSTEM
			SetPendingSpeech( selection.conceptId.c_str(), &selection.Response );
#else
			Assert( selection.pResponse );
			SetPendingSpeech( selection.conceptId.c_str(), selection.pResponse );
#endif
		}
	}
...
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::ShouldSpeakRandom( AIConcept_t conceptId, int iChance )
{
	CAI_AllySpeechManager *	pSpeechManager	= GetAllySpeechManager();
	ConceptInfo_t *			pInfo			= pSpeechManager->GetConceptInfo( conceptId );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::IsAllowedToSpeak( AIConcept_t conceptId, bool bRespondingToPlayer )
{
	CAI_AllySpeechManager *	pSpeechManager	= GetAllySpeechManager();
	ConceptInfo_t *			pInfo			= pSpeechManager->GetConceptInfo( conceptId );
	ConceptCategory_t		category		= ( pInfo ) ? pInfo->category : SPEECH_IDLE;
	...
	{
		if ( CompareConcepts( conceptId, TLK_HELLO ) )
			return false;
		if ( CompareConcepts( conceptId, TLK_HELLO_NPC ) )
			return false;
	}

	if ( !pSpeechManager->ConceptDelayExpired( conceptId ) )
		return false;

	if ( ( pInfo && pInfo->flags & AICF_SPEAK_ONCE ) && GetExpresser()->SpokeConcept( conceptId ) )
		return false;

	if ( !GetExpresser()->CanSpeakConcept( conceptId ) )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::SpeakIfAllowed( AIConcept_t conceptId, const char *modifiers, bool bRespondingToPlayer, char *pszOutResponseChosen, size_t bufsize )
{
	if ( IsAllowedToSpeak( conceptId, bRespondingToPlayer ) )
	{
		return Speak( conceptId, modifiers, pszOutResponseChosen, bufsize );
	}
	return false;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::SpeakIfAllowed( AIConcept_t conceptId, AI_CriteriaSet& modifiers, bool bRespondingToPlayer, char *pszOutResponseChosen, size_t bufsize )
{
	if ( IsAllowedToSpeak( conceptId, bRespondingToPlayer ) )
	{
		return Speak( conceptId, modifiers, pszOutResponseChosen, bufsize );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::OnSpokeConcept( AIConcept_t conceptId, AI_Response *response )
{
	CAI_AllySpeechManager *pSpeechManager = GetAllySpeechManager();
	pSpeechManager->OnSpokeConcept( this, conceptId, response );
