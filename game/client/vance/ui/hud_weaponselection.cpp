...
private:
	...
	CPanelAnimationVar( int, m_iPaintBackgroundType, "PaintBackgroundType", "2" );
...
void CHudWeaponSelection::Paint()
{
	...
	{
		pSelectedWeapon = pPlayer->GetDeployingWeapon() ? pPlayer->GetDeployingWeapon() : pPlayer->GetActiveWeapon();
	}
	else
	{
		switch ( hud_fastswitch.GetInt() )
	...
	if ( ( hud_fastswitch.GetInt() == HUDTYPE_FASTSWITCH || !pPlayer->IsSuitEquipped() ) || hud_fastswitch.GetInt() == HUDTYPE_PLUS )
	{
...
{
	if ( m_iPaintBackgroundType == 0 )
	{
		SetRoundedCorners( 0 );
	}
	else
	{
		SetRoundedCorners( PANEL_ROUND_CORNER_TOP_LEFT | PANEL_ROUND_CORNER_TOP_RIGHT | PANEL_ROUND_CORNER_BOTTOM_LEFT | PANEL_ROUND_CORNER_BOTTOM_RIGHT );
	}
...
void CHudWeaponSelection::OpenSelection( void )
{
	...
	if ( pPlayer->IsSuitEquipped() )
	{
		...
		// Uncomment this out and add the corresponding soundscript if you wish to have it HL1-like.
...
void CHudWeaponSelection::CycleToNextWeapon( void )
{
	...
	m_pLastWeapon = pPlayer->GetDeployingWeapon() ? pPlayer->GetDeployingWeapon() : pPlayer->GetActiveWeapon();
	...
	else
	{
		...
		pNextWeapon = pPlayer->GetDeployingWeapon() ? pPlayer->GetDeployingWeapon() : pPlayer->GetActiveWeapon();
		if ( pNextWeapon )
		{
...
void CHudWeaponSelection::FastWeaponSwitch( int iWeaponSlot )
{
	...
	int iPosition = -1;
	C_BaseCombatWeapon *pActiveWeapon = pPlayer->GetDeployingWeapon() ? pPlayer->GetDeployingWeapon() : pPlayer->GetActiveWeapon();
	...
	// Search for the weapon after the current one and make sure it's in the same bucket.
...
void CHudWeaponSelection::PlusTypeFastWeaponSwitch( int iWeaponSlot )
{
	...
	if ( !bWeaponSelected )
	{
		...
		SetSelectedWeapon( pPlayer->GetDeployingWeapon() ? pPlayer->GetDeployingWeapon() : pPlayer->GetActiveWeapon() );
	}
}
		...
		case HUDTYPE_PLUS:
			{
				...
				{
					// Play the "move weapon selection" sound
					pPlayer->EmitSound( "Player.WeaponSelectionMoveSlot" );
				}
