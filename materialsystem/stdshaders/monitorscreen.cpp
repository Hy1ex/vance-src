...
#include "sdk_unlittwotexture_vs30.inc"
#include "sdk_monitorscreen_ps30.inc"
#include "cpp_shader_constant_register_map.h"

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

BEGIN_VS_SHADER( MonitorScreen, "This is a shader that does a contrast/saturation version of base times lightmap." )
	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
			...
			DECLARE_STATIC_VERTEX_SHADER( sdk_unlittwotexture_vs30 );
			SET_STATIC_VERTEX_SHADER( sdk_unlittwotexture_vs30 );

			DECLARE_STATIC_PIXEL_SHADER( sdk_monitorscreen_ps30 );
			SET_STATIC_PIXEL_SHADER_COMBO( TEXTURE2, (bHasTexture2)?(1):(0) );
			SET_STATIC_PIXEL_SHADER( sdk_monitorscreen_ps30 );
		...
		DYNAMIC_STATE
		{
			...
			DECLARE_DYNAMIC_VERTEX_SHADER( sdk_unlittwotexture_vs30 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, pShaderAPI->GetCurrentNumBones() > 0 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
			SET_DYNAMIC_VERTEX_SHADER( sdk_unlittwotexture_vs30 );

			DECLARE_DYNAMIC_PIXEL_SHADER( sdk_monitorscreen_ps30 );
			SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
			SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, bFullyOpaque && pShaderAPI->ShouldWriteDepthToDestAlpha() );
			SET_DYNAMIC_PIXEL_SHADER( sdk_monitorscreen_ps30 );
		}
		Draw();
	}
END_SHADER
