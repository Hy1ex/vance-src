...
#include "lightmappedpbr_helper.h"
#include "convar.h"
							...
							CBasePerMaterialContextData **pContextDataPtr )
{
	...
	else // not snapshotting -- begin dynamic state
	{
		...
		SET_DYNAMIC_PIXEL_SHADER_COMBO( CUBEMAPCORRECTED, isEnvmapCorrected );
		...
		else
		{
			if ( bUseCSM )
			{
				...
				pShaderAPI->SetPixelShaderConstant( 26, csmData.vecLight.Base() );
				pShaderAPI->SetPixelShaderConstant( 27, csmData.light.Base() );
				pShaderAPI->SetPixelShaderConstant( 28, csmData.ambient.Base() );
				pShaderAPI->SetPixelShaderConstant( 31, biasVar.Base() );
				pShaderAPI->SetPixelShaderConstant( 32, textureSize.Base() );
				pShaderAPI->SetPixelShaderConstant( 33, csmData.sizes.Base() );
			}
		}
	}
	pShader->Draw();
